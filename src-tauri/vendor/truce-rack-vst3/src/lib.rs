...
use vst3::Steinberg::Vst::{
    ...
    IComponentHandlerTrait, IConnectionPoint, IConnectionPointTrait, IEditController,
    IEditControllerTrait, IEventList, IEventListTrait, IHostApplication, IHostApplicationTrait,
    IParamValueQueue, IParamValueQueueTrait, IParameterChanges, IParameterChangesTrait,
    NoteOffEvent, NoteOnEvent, ParamID, ParamValue,
...
use vst3::Steinberg::{
    FUnknown, IBStream, IBStreamTrait, IPlugView, IPlugViewTrait, IPluginBaseTrait,
    IPluginFactory, IPluginFactoryTrait, PClassInfo, PClassInfo_, TUID, ViewRect,
    kNotImplemented, kPlatformTypeHWND, kPlatformTypeNSView, kPlatformTypeX11EmbedWindowID,
...
// ---------------------------------------------------------------------------
// HostContext — minimal IHostApplication + IComponentHandler for plugins.
// Without these, plugins get NULL on initialize() and deadlock or blank-GUI.
// performEdit pushes to a shared queue so parameter changes from the plugin
// GUI reach the component via IParameterChanges in the next process() call.
// ---------------------------------------------------------------------------

type ParamQueue = std::sync::Arc<std::sync::Mutex<Vec<(ParamID, ParamValue)>>>;
...
impl IComponentHandlerTrait for HostContext {
    ...
    unsafe fn performEdit(&self, id: ParamID, value: ParamValue) -> i32 {
        if let Ok(mut q) = self.param_queue.lock() {
            q.push((id, value));
        }
        kResultOk
    }
...
pub struct Vst3Plugin {
    ...
    // Host context — keeps IHostApplication + IComponentHandler alive
    // for the plugin's lifetime. Plugins call QueryInterface on the
    // context pointer during initialize(); without it they get NULL
    // and either crash, blank-GUI, or silently produce no audio.
    _host: ComWrapper<HostContext>,

    /// Shared queue of parameter changes from the plugin GUI (performEdit)
    /// and from the host's set_parameter. Drained in process() and
    /// delivered to the component via IParameterChanges.
    param_queue: ParamQueue,
}

impl Vst3Plugin {
    fn load_from(info: &PluginInfo) -> Result<Self> {
        eprintln!("[vst3-load] step 1: opening module {}", info.path.display());
        let module = unsafe { LoadedModule::open(&info.path) }?;
        eprintln!("[vst3-load] step 2: getting factory");
        let factory = module.factory()?;
        eprintln!("[vst3-load] step 3: parsing cid");
        ...
        eprintln!("[vst3-load] step 3a: creating host context");
        let param_queue: ParamQueue =
            std::sync::Arc::new(std::sync::Mutex::new(Vec::new()));
        let host = ComWrapper::new(HostContext::new(param_queue.clone()));
        let host_ctx = host
            .to_com_ptr::<IHostApplication>()
            .ok_or_else(|| Error::Other("HostContext missing IHostApplication".into()))?;

        eprintln!("[vst3-load] step 4: creating IComponent");
        let component_ptr =
        ...
        eprintln!("[vst3-load] step 5: initializing component");
        if unsafe { component.initialize(host_ctx.as_ptr() as *mut FUnknown) } != kResultOk {
            return Err(Error::LoadFailed {
                path: info.path.clone(),
        ...
        eprintln!("[vst3-load] step 6: casting to IAudioProcessor");
        ...
        eprintln!("[vst3-load] step 7: getting controller");
        ...
        let (controller, separate_controller) = if cls_id_status == kResultTrue {
            eprintln!("[vst3-load] step 7a: separate controller");
            ...
            if unsafe { ctrl_ptr.initialize(host_ctx.as_ptr() as *mut FUnknown) } != kResultOk {
                return Err(Error::LoadFailed {
                    path: info.path.clone(),
            ...
            eprintln!("[vst3-load] step 7b: single component");
        ...
        eprintln!("[vst3-load] step 8: connecting");
        ...
        // Wire the IComponentHandler so the plugin's GUI can call
        // beginEdit/performEdit/endEdit without deadlocking.
        let handler_ptr = host
            .to_com_ptr::<IComponentHandler>()
            .ok_or_else(|| Error::Other("HostContext missing IComponentHandler".into()))?;
        eprintln!("[vst3-load] step 8a: setComponentHandler");
        unsafe {
        ...
        // ShallowHost patch: do not probe `createView("editor")` while loading
        // into the audio chain. Some plugins crash if their editor is created
        // before a real parent HWND exists. Defer view creation to `open()`.
...
impl PluginCore for Vst3Plugin {
    ...
    fn set_parameter(&mut self, index: usize, value: f64) -> Result<()> {
        ...
        // Queue for delivery to component via IParameterChanges in
        // the next process() call. Without this, getState() returns
        // defaults because the component never received the change.
        if let Ok(mut q) = self.param_queue.lock() {
            q.push((info.id, clamped));
        }
        Ok(())
    }
    ...
    fn save_state(&self) -> Result<Vec<u8>> {
        ...
        let data = stream.data.borrow().clone();
        eprintln!("[vst3-save] getState status={status} bytes={}", data.len());
        Ok(data)
    }

    fn load_state(&mut self, bytes: &[u8]) -> Result<()> {
        eprintln!("[vst3-load] load_state input bytes={}", bytes.len());
        ...
        eprintln!("[vst3-load] setState status={status}");
        if status != kResultOk {
            return Err(Error::Other(format!(
                "IComponent::setState returned {status}"
            )));
        }
        // VST3 spec: after component state is restored, sync the
        // controller so the GUI reflects the saved parameters.
        // Without this the controller stays at defaults and the
        // next save overwrites with wrong values.
        let stream2 = ComWrapper::new(MemoryStream {
            data: std::cell::RefCell::new(bytes.to_vec()),
            position: std::cell::Cell::new(0),
        });
        let stream2_ptr = stream2
            .to_com_ptr::<IBStream>()
            .ok_or_else(|| Error::Other("MemoryStream missing IBStream IID".into()))?;
        let status2 = unsafe { self.controller.setComponentState(stream2_ptr.as_ptr()) };
        eprintln!("[vst3-load] setComponentState status={status2}");
        Ok(())
    }
...
impl Plugin<f32> for Vst3Plugin {
    fn process(
        ...
        // Drain pending parameter changes and deliver to the component
        // via IParameterChanges. Without this, changes from set_parameter
        // and performEdit (plugin GUI) never reach the component, and
        // getState() returns default values.
        let pending: Vec<(ParamID, ParamValue)> = self
            .param_queue
            .lock()
            .map(|mut q| q.drain(..).collect())
            .unwrap_or_default();
        let param_changes_wrapper = ComWrapper::new(ParamChangesData::from_changes(&pending));
        let param_changes_ptr = if pending.is_empty() {
            ptr::null_mut::<IParameterChanges>()
        } else {
            param_changes_wrapper
...
// ---------------------------------------------------------------------------
// ParamValueQueueData / ParamChangesData — IParameterChanges impl for
// delivering host-side parameter changes to the component in process().
// ---------------------------------------------------------------------------

/// Single parameter with one value point at sample offset 0.
struct ParamValueQueueData {
    id: ParamID,
    value: ParamValue,
}

impl Class for ParamValueQueueData {
    type Interfaces = (IParamValueQueue,);
}

impl IParamValueQueueTrait for ParamValueQueueData {
    ...
    unsafe fn addPoint(&self, _: i32, _: ParamValue, _: *mut i32) -> i32 {
        -1
    }
}

/// Holds all parameter changes for one process block.
struct ParamChangesData {
    _wrappers: Vec<ComWrapper<ParamValueQueueData>>,
    ptrs: Vec<*mut IParamValueQueue>,
}

impl Class for ParamChangesData {
    type Interfaces = (IParameterChanges,);
}

#[allow(clippy::cast_sign_loss)]
impl IParameterChangesTrait for ParamChangesData {
    unsafe fn getParameterCount(&self) -> i32 {
        i32::try_from(self.ptrs.len()).unwrap_or(0)
    }
    unsafe fn getParameterData(&self, index: i32) -> *mut IParamValueQueue {
        ...
        self.ptrs.get(idx).copied().unwrap_or(ptr::null_mut())
    }
    unsafe fn addParameterData(&self, _: *const ParamID, _: *mut i32) -> *mut IParamValueQueue {
        ptr::null_mut()
    }
}

impl ParamChangesData {
    /// Build from a list of (id, value) pairs. Each parameter gets
    /// its own IParamValueQueue with a single point at offset 0.
    fn from_changes(changes: &[(ParamID, ParamValue)]) -> Self {
        let mut wrappers = Vec::with_capacity(changes.len());
        let mut ptrs = Vec::with_capacity(changes.len());
        for &(id, value) in changes {
            let wrapper = ComWrapper::new(ParamValueQueueData { id, value });
            if let Some(com_ptr) = wrapper.to_com_ptr::<IParamValueQueue>() {
                ptrs.push(com_ptr.as_ptr());
                std::mem::forget(com_ptr);
            }
